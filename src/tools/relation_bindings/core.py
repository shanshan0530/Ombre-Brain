...
async def attach(
    ...
    if is_fixed_relation_type(relation_type):
        if label or reverse_label:
            return "relation_attach 拒绝：固定六种 relation_type 自动反向，不使用 label/reverse_label；自定义关系请使用 custom。"
        forward_label = ""
        backward_label = ""
    else:
        if not label:
    ...
    def mutation(source_post: Any, target_post: Any):
        err = _access(source_post, bucket_id, expected_title)
        if err:
    ...
    result = await rt.bucket_mgr.mutate_relation_pair(bucket_id, target_bucket_id, mutation)
...
async def _change_legacy(
    ...
    def mutation(post: Any):
        ...
        if link["status"] == status:
            return False, f"relation_{action} ok bucket_id={bucket_id} slot={relation_slot} status={status} legacy=true"
        if status == "active":
        ...
        return True, f"relation_{action} ok bucket_id={bucket_id} slot={relation_slot} status={status} legacy=true"

    return (await rt.bucket_mgr.mutate_relation_links(bucket_id, mutation)) or f"未找到桶 {bucket_id}。"


async def _change(bucket_id: str, relation_slot: int, status: str, expected_title: str = "") -> str:
    action = "restore" if status == "active" else "detach"
    bucket_id = str(bucket_id or "").strip()
    expected_title = _normalized_title(expected_title)
    ...
    if not relation_id:
        ...
        return await _change_legacy(bucket_id, expected_title, relation_slot, status)
...
async def detach(bucket_id: str, relation_slot: int, expected_title: str = "") -> str:
    return await _change(bucket_id, relation_slot, "detached", expected_title)


async def restore(bucket_id: str, relation_slot: int, expected_title: str = "") -> str:

...
- 显式记忆检索会先移除“还记得关于……”等会话套话，只把实际主题送入关键词、
  BM25 与向量检索，避免关系称呼压过真正的查询主题。
- 时间、重要度与历史触发次数只负责排列已具备主题证据的候选，不能再把无关但活跃的
  记忆抬过准入线；字面命中、BM25、语义检索与完整桶 ID 直达保持不变。
- 相关的 pinned/permanent 记忆在通过相关性准入后获得小幅排序优先；pinning 本身不能
  为无关记忆创造相关性，也不能绕过准入门槛。

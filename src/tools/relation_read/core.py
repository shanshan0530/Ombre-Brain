...
async def dispatch(
    ...
    if expected_title and _normalized_title(meta.get("title")) != expected_title:
    ...
    for slot, link in visible:
        ...
        if include_titles:
            target = await getattr(rt.bucket_mgr, "get_including_archive", rt.bucket_mgr.get)(
                link["target_bucket_id"]
            )
            if target and _ordinary(target, link["target_bucket_id"]):
                target_title = _normalized_title((target.get("metadata") or {}).get("title"))
                if target_title:
